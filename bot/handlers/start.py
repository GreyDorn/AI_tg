...
from config import FREE_CREDITS_ON_START, REFERRAL_BONUS_CREDITS, DAILY_FREE_CREDITS, MODELS
...
async def cmd_start(message: Message, db_session: AsyncSession, db_user: User) -> None:
    await message.answer(
        ...
        f"🎁 <b>Тебе начислено {FREE_CREDITS_ON_START} бесплатных запросов</b> для знакомства!\n\n"
        f"<b>Что умею:</b>\n"
    ...
    # Создаём первый диалог для нового пользователя
...
async def cmd_help(message: Message, db_user: User) -> None:
    models_text = "\n".join(
        f"• <b>{m.name}</b> — {m.description} ({m.cost_per_message}🔥/msg)"
